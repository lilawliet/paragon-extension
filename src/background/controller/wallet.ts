...
export class WalletController extends BaseController {
  ...
  importWalletConnect = async (
    ...
    if (stashId !== null && stashId !== undefined) {
      ...
      try {
        ...
        const WalletConnectKeyring =
          keyringService.getKeyringClassForType(keyringType);
  ...
  getTransactionHistory = async (address: string) => {
    let result = await openapiService.getAddressRecentHistory(address);
    return result;
  };
  ...
  listChainAssets = async (address: string) => {
    let { confirmed, unconfirmed } = await openapiService.getAddressBalance(
      address
    );
    let amount = (confirmed + unconfirmed) / 10000;
    let assets = [
      { name: COIN_NAME, symbol: COIN_SYMBOL, amount, value: "$6.748.29" },
    ];
    return assets;
  };
