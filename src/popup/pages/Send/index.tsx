    ...
    return(
        <Layout className='h-full'>
                ...
                <Content style={{backgroundColor: '#1C1919'}}>
                    {
                        status == 'create' ? <SendCreate transaction={transaction} setTransaction={setTransaction} setStatus={setStatus}/> : 
                        status == 'confirm' ? <SendConfirm transaction={transaction} setTransaction={setTransaction} setStatus={setStatus}/> : 
                        status == 'sending' ? <Sending transaction={transaction} setTransaction={setTransaction} setStatus={setStatus}/> : 
                        status == 'success' ? <Success transaction={transaction} setTransaction={setTransaction} setStatus={setStatus}/> : <div>error</div>
                    }
                </Content>
                {
                    status !== 'sending'? 
