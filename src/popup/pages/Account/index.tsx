    ...
    return(
        <Layout className='h-full'>
                ...
                <Content style={{backgroundColor: '#1C1919'}}>
                    {
                        status == 'switch' ? <AccountSwitch account={account} setAccount={setAccount} setStatus={setStatus}/> : 
                        status == 'add' ? <AccountAdd account={account} setAccount={setAccount} setStatus={setStatus}/> : 
                        status == 'import' ? <AccountImport account={account} setAccount={setAccount} setStatus={setStatus}/> : 
                        status == 'create' ? <AccountCreate account={account} setAccount={setAccount} setStatus={setStatus}/> : <div>error</div>
                    }
                </Content>
                <Footer style={{height: '5.625rem', backgroundColor: '#1C1919', textAlign: 'center', width: '100%'}}>
                    <Button size='large' type='default' className='box w440' onClick={e => {statusBack()}}>
