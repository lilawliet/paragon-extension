...
import { Panel, getPanel, setPanel } from "@/common/storages/stores/popup/slice"
    ...
    <div className="grid w-full h-full grid-cols-4 text-2xl border-t border-white bg-soft-black border-opacity-10">
      <div
        className={`cursor-pointer flex items-center justify-center h-full text-center ${
          panel == Panel.home ? "text-primary" : ""
        }`}
        onClick={(e) => {
          dispatch(setPanel(Panel.home))
        }}
      >
      ...
      <div
        className={`cursor-pointer flex items-center justify-center h-full text-center ${
          panel == Panel.transaction ? "text-primary" : ""
        }`}
        onClick={(e) => {
          dispatch(setPanel(Panel.transaction))
        }}
      >
      ...
      <div
        className={`cursor-pointer flex items-center justify-center h-full text-center ${
          panel == Panel.settings ? "text-primary" : ""
        }`}
        onClick={(e) => {
          dispatch(setPanel(Panel.settings))
        }}
      >
